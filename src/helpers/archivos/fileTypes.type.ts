...
export class DocumentRequest {
  ...
  @IsEnum(MimeType, {
    message: () =>
      `contentType debe ser uno de los siguientes valores: ${Object.values(MimeType).join(', ')}`,
  })
...
export function IsFileAndMaxSize2MB(
      ...
      validator: {
        validate(value: any, args: ValidationArguments) {
          ...
          detect.fromBuffer(buffer, (err, result) => {
            if (err) {
              logger.info('Error detectando el tipo de archivo:', err);
              return;
            }
            detectedMimeTypeFile = result;
            logger.info('Tipo de archivo detectado:', result);
            return result;
          ...
          const matchedDocumentType = documentTypes.find(
            dt => dt.type === objectInstance.typeFile.toUpperCase()
          );
